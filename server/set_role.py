...
所以组里没人能靠抢注或者调接口把自己变成导师。
    ...
    python3 set_role.py <用户名> advisor  设为导师
...
def main(argv):
    ...
    if len(argv) == 0:
        rows = c.execute("SELECT username, display_name, role, created_at FROM users"
                         " ORDER BY role DESC, created_at").fetchall()
        if not rows:
            print("还没有人注册")
            return 0
        ...
        print("-" * 62)
        for r in rows:
            mark = "★" if r["role"] == "advisor" else " "
            when = time.strftime("%Y-%m-%d %H:%M", time.localtime(r["created_at"] / 1000))
            print(f"{mark}{r['username']:<19} {r['display_name']:<16} {r['role']:<9} {when}")
        return 0
