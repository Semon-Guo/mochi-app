 ...
 */
import { stampChanges, mergeIncoming, pendingCount, SYNC_KINDS } from "./sync.js";
...
console.log("\n── stampChanges ──");
{
  ...
  const out = stampChanges(prev, next, 5000);
  chk("新增记录会打戳", out._sync.stamps.r1?.at === 5000 && out._sync.stamps.r1?.t === "records",
      JSON.stringify(out._sync.stamps.r1));
}
{
  const prev = stampChanges(base(), { ...base(), records: [rec("r1", "旧")] }, 5000);
  const next = { ...prev, records: [rec("r1", "新")] };
  const out = stampChanges(prev, next, 7000);
  chk("修改记录会更新戳", out._sync.stamps.r1.at === 7000, String(out._sync.stamps.r1.at));
}
{
  const prev = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
  const next = { ...prev, records: [] };
  const out = stampChanges(prev, next, 8000);
  chk("删除记录会留墓碑", out._sync.tombs.r1?.at === 8000 && out._sync.tombs.r1?.t === "records");
  chk("删除后不再保留戳", !out._sync.stamps.r1);
}
{
  const prev = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
  const out = stampChanges(prev, prev, 9000);
  chk("无变化时不产生新对象（否则 setData 会无限循环）", out === prev);
}
{
  const prev = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
  const next = { ...prev, todos: [{ id: "t1", text: "写代码" }], notes: [{ id: "n1" }] };
  const out = stampChanges(prev, next, 9000);
  chk("待办和笔记不参与同步", !out._sync.stamps.t1 && !out._sync.stamps.n1,
      Object.keys(out._sync.stamps).join(","));
}
{
  const prev = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
  const revived = { ...prev, records: [] };
  const dead = stampChanges(prev, revived, 6000);
  const back = stampChanges(dead, { ...dead, records: [rec("r1", "复活")] }, 7000);
  chk("删除后重新出现会清掉墓碑", !back._sync.tombs.r1 && !!back._sync.stamps.r1);
}

console.log("\n── pendingCount ──");
{
  const d = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
...
{
  let d = stampChanges(base(), { ...base(), records: [rec("r1", "本地版本")] }, 5000);
...
{
  let d = stampChanges(base(), { ...base(), records: [rec("r1", "本地未推送的改动")] }, 9000);
  const out = mergeIncoming(d, freshSync(d), {
...
{
  let d = stampChanges(base(), { ...base(), records: [rec("r1", "x")] }, 5000);
  d._sync.pushed.r1 = 5000;
  const out = mergeIncoming(d, freshSync(d), {
